...
DB_URL = "sqlite:///C:/Users/100ca/finance_local.db"
...
def main():
    ...
    with Session(engine) as session:
        ...
        for tx in transactions:
            # We can check duplicates, but since v2 db is fresh, just insert.
            # But the loop re-inserts if we re-run.
             session.add(tx)
             count += 1
        
        session.commit()
        print(f"Imported {count} new transactions to {DB_URL}.")
