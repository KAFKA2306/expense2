...
class ExpenseAnalysisService:
    ...
    def __init__(self, data_path: Optional[Path] = None):
        data_path = data_path or (Path(__file__).parent.parent.parent / "data" / "transactions.csv")
    ...
    def filter_expenses(self, df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        df = df if df is not None else self._load_data()
    ...
    def get_category_breakdown(self, year_month: Optional[str] = None) -> list[dict]:
        df = self.filter_expenses()
        df = df[df["year_month"].astype(str) == year_month] if year_month else df
        breakdown = df.groupby("category")["amount"].sum().abs().sort_values(ascending=False).reset_index()
    ...
    def get_fixed_vs_variable(self) -> dict:
        ...
        fixed_items = df[fixed_mask].groupby("description")["amount"].mean().abs().sort_values(ascending=False).head(10).reset_index().to_dict("records")
        return {"fixed": round(fixed_total, 0), "variable": round(variable_total, 0), "fixed_items": fixed_items}

    def get_reducible_expenses(self) -> list[dict]:
        df = self.filter_expenses()
        reducible = df[df["category"].isin(self.REDUCIBLE_CATEGORIES)]
        avg_by_category = (reducible.groupby("category")["amount"].sum().abs() / reducible["year_month"].nunique()).round(0).reset_index()
    ...
    def get_merchant_ranking(self, top_n: int = 10) -> list[dict]:
        df = self.filter_expenses()
        ranking = df.groupby("description").agg({"amount": ["sum", "count"]}).droplevel(0, axis=1).reset_index()
    ...
    def get_mom_yoy_comparison(self) -> dict:
        ...
        def get_category_totals(period):
            return df[df["year_month"] == period].groupby("category")["amount"].sum().abs()

        last_m, prev_m, last_y = get_category_totals(last_month), get_category_totals(prev_month), get_category_totals(last_year_month)

        mom = [{"category": cat, "current": last_m.get(cat, 0), "previous": prev_m.get(cat, 0), "change": round((last_m.get(cat, 0) - prev_m.get(cat, 0)) / prev_m.get(cat, 0) * 100, 1) if prev_m.get(cat, 0) > 0 else 0} for cat in last_m.index]
        yoy = [{"category": cat, "current": last_m.get(cat, 0), "previous": last_y.get(cat, 0), "change": round((last_m.get(cat, 0) - last_y.get(cat, 0)) / last_y.get(cat, 0) * 100, 1) if last_y.get(cat, 0) > 0 else 0} for cat in last_m.index]
    ...
    def get_moving_average_12m(self) -> list[dict]:
        ...
        return pd.DataFrame({"month": monthly.index.astype(str), "actual": monthly.values, "ma12": ma12.values.round(0)}).to_dict("records")
    ...
    def get_annual_forecast(self) -> dict:
        ...
        ytd_projection = (ytd / months_elapsed) * 12
        ma_projection = monthly_avg * 12
        forecast = ytd_projection * 0.6 + ma_projection * 0.4
        return {"ytd": round(ytd, 0), "ytd_projection": round(ytd_projection, 0), "ma_projection": round(ma_projection, 0), "forecast": round(forecast, 0), "monthly_avg": round(monthly_avg, 0), "months_elapsed": months_elapsed}
